      ...
      while (enumAtts.hasMoreElements()) {
		...
		for (int i=0; i<m_Instances.numClasses(); i++){
			if (attribute.isNominal()){
				DiscreteEstimator de=((DiscreteEstimator)(nb.m_Distributions[attIndex][i]));
				for (int j=0; j<de.getNumSymbols(); j++){
					m_Distributions[attIndex][i].
					addValue(j, de.getCount(j));
				}
			}
			else{
	//			System.out.println("numberic");
				NormalEstimator ne=((NormalEstimator)(nb.m_Distributions[attIndex][i]));
				m_Distributions[attIndex][i].
				addValue(ne.getMean(), ne.getSumOfWeights());
			}
		}
      ...
      DiscreteEstimator de=(DiscreteEstimator)nb.m_ClassDistribution;
      for (int i=0; i<m_Instances.numClasses(); i++){
    	  m_ClassDistribution.addValue(i,de.getCount(i));
      }
    
  }
  
  public void mergeClasses(int c1, int c2){//c1 is removed and added to c2
	  Enumeration enumAtts = m_Instances.enumerateAttributes();
      int attIndex = 0;
      while (enumAtts.hasMoreElements()) {
		Attribute attribute = (Attribute) enumAtts.nextElement();
			...
			if (attribute.isNominal()){
				DiscreteEstimator de=((DiscreteEstimator)(m_Distributions[attIndex][c1]));
				for (int j=0; j<de.getNumSymbols(); j++){
					m_Distributions[attIndex][c2].addValue(j, de.getCount(j));
				}
				m_Distributions[attIndex][c1] = new DiscreteEstimator(attribute.numValues(), true);
			}
			else{
	//			System.out.println("numberic");
				NormalEstimator ne = ((NormalEstimator)(m_Distributions[attIndex][c1]));
				m_Distributions[attIndex][c2].addValue(ne.getMean(), ne.getSumOfWeights());
				m_Distributions[attIndex][c1]=new NormalEstimator(((NormalEstimator)m_Distributions[attIndex][c2]).getPrecision());
			}
      ...
      DiscreteEstimator de = (DiscreteEstimator)m_ClassDistribution;
	  de.addValue(c2,de.getCount(c1));
	  de.setMyParamsInMerge(c1);
	...
	public double classifyInstance(Instance instance) throws Exception {
		...
		case Attribute.NOMINAL:
			...
			if (max > 0) {
				...
				System.out.println("oomad inja!!! missingValue ro chi konam?!");
